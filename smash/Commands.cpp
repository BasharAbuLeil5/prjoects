...
void JobsCommand::execute(){
  // printing the jobs of the current smash or printing the global background running jobs?
  /*SmallShell& smash=SmallShell::getInstance();
  smash.printSmashJobs();*/
  m_jobsList->removeFinishedJobs();
...
void JobsList::removeFinishedJobs(){// bashar writed this function
    ...
    /*while(!finishedJobs.empty()){
        removeJobById(finishedJobs.top());
        finishedJobs.pop();
    }*/
...
/*we have to implement the following operations:
    isNumber(int num); done
    createString(char**arguments);//done
    createCommand();
    executeCommand();  //done
    + shell funcitons c'tor d'tor and etc...

*/

/*
We have to go through fg command and redirection command.
*/
